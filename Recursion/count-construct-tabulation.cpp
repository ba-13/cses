...
int main()
{
    ...
    /**
     * Number of ways the target can be created by concatenating words from the wordBank
     */
    int n;
    std::string target;
    std::cin >> target >> n;
    ...
    result[0] = 1; // "" can be made one way

    fr(i, target.size() + 1)
    {
        if (result[i] == 0)
            continue; // not a possible path
        fr(j, wordBank.size())
        {
            // making sure index within bounds
